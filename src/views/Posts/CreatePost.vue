...
<template>
  <transition name="modal">
    <div v-if="showModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-60 z-50 p-4">
      <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-2xl w-full max-w-md max-h-[85vh] overflow-y-auto transform transition-all duration-300">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700">
          <h2 class="text-2xl font-bold text-gray-900 dark:text-gray-100 tracking-tight">Create a Post</h2>
          <button @click="$emit('close')" class="p-1 text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors duration-200">
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="relative">
            <label class="flex items-center text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 cursor-pointer transition-colors duration-200">
            ...
            <div v-if="imagePreview" class="mt-4 relative group">
              <img :src="imagePreview" class="w-full rounded-lg shadow-md object-cover max-h-64 transition-transform duration-300 group-hover:scale-105" alt="Preview" />
              <button
                @click="removeImage"
        ...
        <div class="flex justify-end px-6 py-4 border-t border-gray-200 dark:border-gray-700 bg-gray-100 dark:bg-gray-800">
          <button
            @click="$emit('close')"
