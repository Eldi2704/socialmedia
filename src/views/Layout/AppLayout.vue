...
const posts = ref<any[]>([]); // funksion qe ruan një listë postimesh qe mbushet pas postimeve
const showModal = ref(false);
const showMessagesModal = ref(false);// Kontrollon nëse një modal është i dukshem apo jo
...
const fetchUserForComment = async (userId: number): Promise<User> => { // FUNKSION ASINKRON
      ...
      posts.value = await Promise.all(
        postData.map(async (post: any) => {
          ...
          return {
            ...
            isLiked:
              post.likes?.some(
                (like: any) => like.user_id === user.value?.id
              ) || false,
...
const toggleLike = async (postId: number) => {
  ...
  postItem.likes_count = postItem.isLiked
    ? postItem.likes_count + 1
    : Math.max(0, postItem.likes_count - 1);
...
const toggleMessagesModal = () =>
  (showMessagesModal.value = !showMessagesModal.value);
...
<template>
  <div
    class="flex w-full min-h-screen bg-gray-900 text-white dark:border-gray-700"
  >
    <link
      rel="stylesheet"
    ...
    <main
      class="flex-1 p-8 max-w-3xl mx-auto md:mx-0 md:ml-[20%] lg:ml-[23%] lg:mr-[27%]"
    >
      <div
        class="bg-gradient-to-r from-black-800 to-gray-900 p-4 rounded-2xl shadow-xl flex items-center cursor-pointer mb-8 transition-all duration-300 hover:shadow-2xl hover:to-gray-900 group"
      ...
      >
        <img
          ...
          :src="
            user?.avatar ||
            'http://lilithaengineering.co.za/wp-content/uploads/2017/08/person-placeholder.jpg'
          "
          alt="User Avatar"
        />
        <div
          class="flex-grow bg-gray-700 text-gray-400 px-5 py-3 rounded-xl text-lg font-medium transition-colors duration-300"
        >
          What's on your mind?
        </div>
      </div>
      ...
      <div v-if="posts.length" class="space-y-8">
        <div
          v-for="post in posts"
          :key="post.id"
          class="bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-700"
        >
          ...
          <div class="post-content">
            <div
              v-if="post.content && post.content.image"
          ...
          >
            <div class="flex space-x-6">
              <button
                ...
                class="flex items-center text-gray-400 hover:text-red-400"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6"
              ...
              <button
                ...
                class="flex items-center text-gray-400 hover:text-blue-400"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6"
              ...
              <button
                class="flex items-center text-gray-400 hover:text-green-400"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  class="h-6 w-6"
